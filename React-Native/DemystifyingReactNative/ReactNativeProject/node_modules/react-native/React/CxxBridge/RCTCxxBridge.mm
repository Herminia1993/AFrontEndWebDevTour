...
- (void)start
{
  ...
  // 注册非自动导出的 module（在程序启动时，有些模块会自动注册，这里是那些没有自动注册的模块）
  ...
  [self ensureOnJavaScriptThread:^{
    // 初始化 bridge 和 JSExecutor
    [weakSelf _initializeBridge:executorFactory];
    dispatch_group_leave(prepareBridge);
  }];

  // 异步从本地或者服务器加载 JS 资源，并暂存起来
...
#if RCT_DEBUG
  // 检查时是否有模块，遵循了 RCTBridgeModule 协议但是没有注册（也就是使用了 RCT_EXPORT_MODULE() 宏）
...
- (void)_runAfterLoad:(RCTPendingCall)block
{
  ...
  // 如果 JS 资源还在加载或者队列里面还有待执行的任务，就先把这个任务加到队列中
